...
exports.add_comment = [
  ...
  async (req, res) => {
    ...
    try {
      ...
      story.comments.unshift(new_comment._id);
      const comment_sacved =await story.save();
      res.status(201).json({ status:true,message: "Comment added successfully",data:comment_sacved });
    } catch (err) {
...
exports.get_comments = [
   // login_validator,
    async (req, res) => {
        ...
        res.status(200).json({ status:true,message: "Comments list",data:comments });
        } catch (err) {
        console.error(err.message);
