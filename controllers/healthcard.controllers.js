...
exports.update_healthcard = [
  ...
  async (req, res) => {
    try {
      ...
      console.log("line 32", documents_attached);
      const documents_attached_url = await awsS3.multiple_file_upload(documents_attached);
      console.log("line 68", documents_attached_url);
      ...
      // Update document fields if files are provided
      if (files.document_attached) {
        updateObject.document_attached = {
          document_name: documents_attached.map((file) => file.originalname),
          document_url: documents_attached_url.map((file) => file.Location),
        };
      }
